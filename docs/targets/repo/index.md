...
- The `--depth=N` flag performs a shallow clone/pull with the specified history depth (e.g. `--depth=5`). For existing non-shallow repositories, depth is only effective for pull operations on repositories that were originally cloned with a depth limit.
