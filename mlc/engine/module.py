...
from mlc.automation import Automation
from mlc.cache_action import CacheAction
import mlc.utils as utils
from mlc.engine.utils import *
from mlc.engine.script_utils import *
from mlc.engine.cache_utils import *
...
class ScriptAutomation(Automation):
    ...
    ############################################################
    def __init__(self, action_object, run_args={}):
        # Engine now lives inside mlcflow (mlc/engine/). The automation-type
        # meta (alias: script, uid: 5b4e0237da074764) ships alongside the
        # engine as mlc/engine/meta.json, so self.path/self.meta are derived
        # from the engine package itself rather than a dynamically-loaded
        # module path.
        super().__init__(action_object, "script", __file__)
        self.os_info = {}
...
"""
    ...
    def doc(self, i):
        ...
        from mlc.engine.doc import generate_doc
    ...
    def help(self, i):
        ...
        from mlc.engine.help import display_help
        return display_help(self, i)

    ############################################################

    def lint(self, i):
        from mlc.engine.lint import lint_meta
        return lint_meta(self, i)

    ############################################################

    def validate(self, i):
        from mlc.engine.validate import validate_scripts
        return validate_scripts(self, i)

    ############################################################

    def dockerfile(self, i):
        from mlc.engine.docker import dockerfile
        return dockerfile(self, i)

    ############################################################
    def docker(self, i):
        from mlc.engine.docker import docker_run
        return docker_run(self, i)

    ############################################################

    def apptainerfile(self, i):
        from mlc.engine.apptainer import apptainerfile
        return apptainerfile(self, i)

    ############################################################
    def apptainer(self, i):
        from mlc.engine.apptainer import apptainer_run
        return apptainer_run(self, i)
    ############################################################

    def experiment(self, i):
        from mlc.engine.experiment import experiment_run
        return experiment_run(self, i)

    ############################################################
    def remote_run(self, i):
        from mlc.engine.remote_run import remote_run
    ...
    def clean_some_tmp_files(self, i):
        ...
        if cur_work_dir != '' and os.path.isdir(cur_work_dir):
            for x in ['tmp-run.bat', 'tmp-run.sh', 'tmp-state.json']:
