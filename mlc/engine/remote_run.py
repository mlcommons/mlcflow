...
def remote_run(self_module, i):
    ...
    if i.get('remote_copy_mlc_repos', False):
        # Respect MLC_REPOS (via the resolved repos_path) instead of hardcoding
        # ~/MLC/repos, so a non-default local repo root is copied correctly.
        local_repos_path = getattr(
            self_module.action_object, 'repos_path', None) or os.path.join(
