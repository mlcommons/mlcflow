...
- Check whether CI actually runs for the change: compare the touched paths
  against each workflow's `paths:` filter. `automation/**` *is* covered by
  `test-unit.yml` now, but the per-script workflows in `mlperf-automations`
  key off a `tests:` block in `meta.yaml` — a script without one runs nothing.
...
- Don't hard-code `~/MLC/repos` — there are two independent roots, and both are per-environment (`~/MLC/envs/<hash of site-packages>`) when `mlc-scripts` is installed. Script content goes under `self.repos_path` (from `MLC_REPOS`); caches, generated scripts and docker/apptainer contexts go under `self.local_cache_path` / `self.cache_path` (from `MLC_CACHE`, or from the registered `local` repo when it is unset).
- Don't build a cache path out of `self.repos_path`, **even though the two roots are usually equal**. On a plain packaged install they resolve to the same directory, so this mistake now *works* — and then breaks only for users who set `MLC_CACHE` or whose `local` repo came from the registry. The rule does not rest on the roots differing: the cache root is the one that owns the `local` repo, and it is the only one authorised to answer. A path built from the wrong root is a plausible-looking directory that no repo is registered under, so nothing errors.
...
- Don't `shutil.rmtree()` or write to a path that came out of a search result without checking `Action._packaged_path()` first. Search results include the installed `mlc-scripts`, so the path may be inside `site-packages`, which pip owns. Match by path containment, never by alias or uid — a pulled checkout shares both with the packaged copy, and deleting the checkout must keep working.
- Don't guard a destructive loop from inside the loop. `Action.rm()` deletes as it iterates, so the packaged-repo check runs over the whole result set before the first `rmtree` — a mid-loop `return` would leave the earlier matches already gone.
