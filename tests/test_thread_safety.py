...
class ConcurrentRmCacheTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Both roots, not just MLC_REPOS: the cache root is moved only
        # by MLC_CACHE, so pinning the repo root alone would leave this
        # test writing cache items into the real ~/MLC/repos/local.
        os.environ["MLC_REPOS"] = repos_dir
        os.environ["MLC_CACHE"] = repos_dir
    ...
    def _restore_env(self):
        for name, previous in (("MLC_REPOS", self.previous_mlc_repos),
                               ("MLC_CACHE", self.previous_mlc_cache)):
            if previous is None:
...
class ConcurrentReposJsonTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ["MLC_CACHE"] = self.repos_dir
