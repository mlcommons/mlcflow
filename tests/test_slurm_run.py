...
class TestRemoteRunIsolation(unittest.TestCase):
    ...
    def test_remote_isolated_sets_tmp_mlc_repos_and_cleanup(self):
        ...
        # MLC_CACHE too, and this is not redundant: the two roots resolve
        # independently, so MLC_REPOS alone leaves the cache in the remote's
        # real home -- outliving the trap below, and leaving the copy-back
        # path empty. Verified on a real node before this assertion was added.
        self.assertRegex(
            combined,
            r'export MLC_CACHE="/tmp/mlcflow-isolated-[0-9a-f]+/MLC"')
    ...
    def test_remote_isolated_cache_root_matches_copy_back_source(self):
        """The isolated cache root and the copy-back source must be one dir.

        These are set in two places that nothing links, which is exactly why
        the mismatch was silent: the preamble exported only MLC_REPOS, so the
        cache resolved under the remote's real home while the copy-back kept
        reading {tmp}/MLC/local/cache. That path just stayed empty -- the run
        reported success and copied back nothing. Asserting the two agree is
        ...
        exports = [c for c in captured['run_cmds']
                   if c.startswith('export MLC_CACHE=')]
        self.assertEqual(
            len(exports), 1,
            f"Expected exactly one MLC_CACHE export, got: {exports}")
        cache_root = exports[0].split('=', 1)[1].strip('"')

        # Cache items resolve to <cache root>/local/cache; confirmed against a
        # real remote node, not inferred from the resolution code.
        expected_source = f'{cache_root}/local/cache'
        staging = [c for c in captured.get('post_run_cmds', [])
                   if 'cp -a' in c]
        self.assertEqual(
            len(staging), 1,
            f"Expected one staging command, got: {captured.get('post_run_cmds')}")
        self.assertIn(
            expected_source, staging[0],
            f"Copy-back reads a different dir than the preamble isolates.\n"
            f"  isolated cache root: {cache_root}\n"
            f"  copy-back command:   {staging[0]}")

    def test_remote_isolated_venv_is_inside_the_dir_the_trap_removes(self):
        """An isolated run must not reuse the venv in the remote user's home.

        This path deliberately does not cd, so the default bare venv name
        'mlcflow' resolved against the ssh login directory -- meaning an
        isolated run activated, and --remote_mlc_scripts pip-installed into,
        whatever venv the operator already had there. Asserting containment
        against the trap rather than a literal path, because the guarantee is
        "cleaned up with everything else", not any particular directory.
        """
        cmds = self._capture_remote_run_cmds(remote_isolated=True)

        installer = [c for c in cmds if '--venv-dir' in c]
        self.assertEqual(len(installer), 1, cmds)
        venv = installer[0].split('--venv-dir', 1)[1].split()[0]

        trap = [c for c in cmds if c.startswith('trap ')]
        self.assertEqual(len(trap), 1, cmds)
        removed = trap[0].split('rm -rf ', 1)[1].split('"')[0].strip()

        self.assertTrue(
            venv.startswith(removed + '/'),
            f"venv escapes cleanup.\n  venv:    {venv}\n  removed: {removed}")

    def test_remote_isolated_explicit_venv_still_wins(self):
        """--remote_python_venv is the caller being specific; isolation must
        not silently override it."""
        cmds = self._capture_remote_run_cmds(
            remote_isolated=True, remote_python_venv='/opt/shared/mlcflow')
        installer = [c for c in cmds if '--venv-dir' in c]
        self.assertEqual(len(installer), 1, cmds)
        self.assertIn('--venv-dir /opt/shared/mlcflow', installer[0])

    def test_non_isolated_venv_name_is_unchanged(self):
        """The opt-in promise: without --remote_isolated nothing moves."""
        cmds = self._capture_remote_run_cmds()
        installer = [c for c in cmds if '--venv-dir' in c]
        self.assertEqual(len(installer), 1, cmds)
        self.assertIn('--venv-dir mlcflow', installer[0])
    ...
    def test_remote_isolated_copy_directory_stays_relative_for_rsync(self):
        ...
        only the MLC roots (MLC_REPOS and MLC_CACHE) are redirected to
        the isolated dir.
        """
        from unittest.mock import patch, MagicMock
        from script.remote_run import remote_run
    ...
    def test_remote_isolated_repos_copy_directory_stays_relative_for_rsync(
        ...
        self.assertFalse(copy_dir.startswith('/'),
                         f"copy_directory should be relative (for rsync), got: {copy_dir!r}")
        # The run_cmds must make mlcflow find the copied repos on the remote.
        # This used to be a symlink into $MLC_REPOS; it is now a registration
        # step, which works whether or not MLC_REPOS is exported over there
        # (see docs/design/per-environment-roots.md, decision 12). Isolated
        # mode exports it, so the registration lands in the isolated root.
        run_cmds = captured.get('run_cmds', [])
        combined = " ; ".join(run_cmds)
        self.assertIn('MLC/repos', combined)
        self.assertIn('mlc add repo', combined)

        # The registration needs the CLI, so it must come after the installer
        # rather than in the isolated-mode preamble.
        def index_of(needle):
            return next(n for n, c in enumerate(run_cmds) if needle in c)

        installer = index_of('installer')
        register = index_of('mlc add repo')
        self.assertGreater(register, installer,
                           "repos registered before mlc exists on the remote")
...
class TestSlurmRunInputNormalization(unittest.TestCase):
    ...
    def test_slurm_isolated_sets_tmp_mlc_repos_and_cleanup(self):
        ...
        # See the note in the remote-isolated test: MLC_REPOS alone does not
        # move the cache root.
        self.assertIn('export MLC_CACHE="$PWD/MLC"', bash_c_cmd)
        self.assertIn(
