        ...
        if k.startswith('+'):
            ...
            env_value = f"""{
                ...
                os_info['env_var'].replace(
                    'env_var', key)}"""
        elif not is_windows and isinstance(env_value, list):
            # On Unix, list values cannot be safely represented as a shell
            # variable: Python's repr() uses single quotes and may embed
            # shell command-substitution syntax ($(...)) that bash
            # misparses inside a double-quoted export statement.
            # customize.py reads these values directly from the Python env
            # dict, so skipping them in the shell script is safe.
            continue
