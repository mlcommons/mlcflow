...
def remote_run(self_module, i):
    ...
    if remote_isolated:
        ...
        # The venv has to be named explicitly here, because this path
        # deliberately does not cd into the isolated dir (see below) and the
        # default venv argument is the bare relative name 'mlcflow' -- which
        # the installer resolves against the ssh login directory, i.e. the
        # remote user's home. Left alone, an isolated run reuses (and
        # --remote_mlc_scripts pip-installs into) whatever venv already sits
        # there, which is the opposite of isolation and is what the
        # --remote_no_internet error above already claims we do not do.
        # slurm_run gets this for free because it does cd. An explicit
        # --remote_python_venv still wins.
        if not i.get('remote_python_venv'):
            remote_mlc_python_venv = f'{_remote_tmp_dir}/venv'
        ...
        preamble.extend([
            ...
            # Both roots, not just MLC_REPOS. The two are resolved
            # independently, so exporting MLC_REPOS alone leaves the cache in
            # the remote's real home: it outlives the trap below, and the
            # copy-back reads {_remote_tmp_dir}/MLC/local/cache, which would
            # be empty. Pointing MLC_CACHE at the same dir puts the local repo
            # (and so the cache) exactly where the copy-back looks for it.
            f'export MLC_REPOS="{_remote_tmp_dir}/MLC"',
            f'export MLC_CACHE="{_remote_tmp_dir}/MLC"',
    ...
    # First point at which `mlc` is on PATH. Commands that need the CLI
    # itself - registering copied repos, below - are inserted here rather
    # than at run_cmds_start_index, which sits before the installer.
    run_cmds_after_venv = len(run_cmds)
    ...
    if i.get('remote_copy_mlc_repos', False):
        ...
        if repo_files:
            ...
            # In isolated mode the repos are still rsynced to the relative
            # path (home-dir-relative), the same as non-isolated. The isolated
            # dir is not created until the command payload runs, so pointing
            ...
            # own interpreter, so a path derived from this machine's
            # site-packages means nothing over there. Registering also does
            # the right thing under --remote_isolated, where MLC_REPOS points
            # at the throwaway dir.
            run_cmds.insert(run_cmds_after_venv,
                            f'for d in "{remote_mlc_repos_path_for_cmd}"/*/ ; do '
    ...
    if remote_copy_back_mlc_cache:
        ...
        if remote_copy_back_mlc_cache_path:
            ...
            path_to_copy_back_files = get_local_mlc_cache_path(self_module)
